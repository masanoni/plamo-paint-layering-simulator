...
const AdminPanel: React.FC<AdminPanelProps> = ({ paints, onUpdate, isVisible, onClose, apiKey }) => {
  ...
  const generateLinksForPaint = (paint: Paint): { amazonUrl: string, rakutenUrl: string } => {
    ...
    // Generate Rakuten Link
    const encodedProductName = encodeURIComponent(productName);
    const rakutenUrl = `https://search.rakuten.co.jp/search/mall/${encodedProductName}/`;

    return { amazonUrl: amazonUrl.toString(), rakutenUrl };
  };

  const handleGenerateAndApplyLinks = (paint: Paint) => {
      const { amazonUrl, rakutenUrl } = generateLinksForPaint(paint);
      const newPaints = editablePaints.map(p => {
          if (p.code === paint.code) {
              return { ...p, amazonUrl, rakutenUrl };
          }
          return p;
      });
  ...
  const handleGenerateAllMissingLinks = () => {
      if (!window.confirm('Amazonまたは楽天のURLが空のすべての塗料に対して、アフィリエイト検索リンクを自動生成します。既存のURLは上書きされません。よろしいですか？')) {
          return;
      }
      const newPaints = editablePaints.map(paint => {
          const shouldUpdateAmazon = !paint.amazonUrl;
          const shouldUpdateRakuten = !paint.rakutenUrl;

          if(shouldUpdateAmazon || shouldUpdateRakuten) {
              const { amazonUrl, rakutenUrl } = generateLinksForPaint(paint);
              return {
                  ...paint,
                  amazonUrl: shouldUpdateAmazon ? amazonUrl : paint.amazonUrl,
                  rakutenUrl: shouldUpdateRakuten ? rakutenUrl : paint.rakutenUrl,
              };
          }
          return paint;
      });
      setEditablePaints(newPaints);
      alert('リンクの生成が完了しました。「ローカルに変更を保存」を押して変更を確定してください。');
  };
      ...
      >
        <header className="flex justify-between items-center p-4 border-b border-slate-700 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-end">
            <button onClick={handleGenerateAllMissingLinks} className="px-3 py-2 text-sm font-bold text-green-300 bg-green-800 rounded-md hover:bg-green-700 transition-colors">欠けているリンクを一括生成</button>
            ...
            {editablePaints.map(paint => (
              <div key={paint.code} className="bg-slate-800 p-3 rounded-lg">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  />
                  <button onClick={() => handleGenerateAndApplyLinks(paint)} className="px-3 py-1 text-xs font-bold text-white bg-teal-600 rounded-md hover:bg-teal-700 transition-colors" title="アフィリエイト検索リンクを生成します。既存のリンクは上書きされます。">
                    生成
                  </button>
                </div>
              </div>
